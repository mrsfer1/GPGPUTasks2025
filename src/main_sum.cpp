            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int current_size = (n - 1) / GROUP_SIZE + 1;
                        int t = 0;
                        while (current_size > 1){
                            if (t % 2 == 0){
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), reduction_buffer1_gpu, reduction_buffer2_gpu, current_size);
                            } else {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), reduction_buffer2_gpu, reduction_buffer1_gpu, current_size);
                            }
                            ++t;
                            current_size = (current_size - 1) / GROUP_SIZE + 1;
                        }
                        if (t % 2 == 0){
